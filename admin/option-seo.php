...
$(function(){
    ...
    $("#buildmap").click(function(){
        var apicontent=$('[name="maptype"]').val();
        $.post('api.php',{"action":"sitemap","content":apicontent},function(data){
            alert(data);
            var uri='http://<?php echo $_SERVER['HTTP_HOST'] ?>/sitemap.'+apicontent;
            var htmlvar='查看<a href="'+uri+'" target="_blank">'+uri+'</a>';
            $("span.result-info").html(htmlvar);
        });
        //alert($('[name="maptype"]').val());
    });
});
</script>
<div class="clear">&nbsp;</div>
